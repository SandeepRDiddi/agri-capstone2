...
db=mysql.connector.connect(user="root",password="",port='3306',database='agriculture')
cur=db.cursor()
...
def model():
    if request.method == "POST":
        ...
        if s == 0:
            ...
            ac_nb = accuracy_score(y_test, y_pred)
            ac_nb = ac_nb * 100
            msg = 'The accuracy obtained by Catboost Classifier is ' + str(ac_nb) + str('%')
            return render_template('model.html', msg=msg)
        elif s == 7:
            ...
            ac_nb = accuracy_score(y_test, y_pred)
            ac_nb = ac_nb * 100
            msg = 'The accuracy obtained by Isolation Forest is ' + str(ac_nb) + str('%')
            return render_template('model.html', msg=msg)
...
def datacleaning():
    ...
    if request.method == "POST":
       ...
       # Assigning the value of x and y 
        ...
        a_train, a_test, b_train, b_test = train_test_split(a,b,test_size=0.4, random_state=101)
...
def modeltraining():
    if request.method == "POST":
        ...
        if s == 0:
            ...
            from sklearn.tree import DecisionTreeClassifier
            dt = DecisionTreeClassifier()
            dt.fit(a_train[:100],b_train[:100])
            b_pred = dt.predict(a_test[:100])
            ac_dt = accuracy_score(b_test[:100], b_pred)
            ac_dt = ac_dt * 100
            print('aaaaaaaaaaaaaaaaaaaaaaaaa')
            msg = 'The accuracy obtained by Decision Tree Classifier is  ' + str(ac_dt) + str('%')
            ...
            from sklearn.ensemble import RandomForestClassifier
            classifier = RandomForestClassifier()
            classifier.fit(a_train[:100], b_train[:100])
            y_pred  =  classifier.predict(a_test[:100])            
            
            ac_nb = accuracy_score(b_test[:100], y_pred)
            ac_nb = ac_nb * 100
            msg = 'The accuracy obtained by Random Forest Classifier is ' + str(ac_nb) + str('%')
            return render_template('modeltraining.html', msg=msg)
        elif s == 3:
            ...
            lr.fit(a_train[:100], b_train[:100])
            y_pred  =  lr.predict(a_test[:100])            
            ac_nb = accuracy_score(b_test[:100], y_pred)
            ac_nb = ac_nb * 100
            msg = 'The accuracy obtained by LogisticRegression  is ' + str(ac_nb) + str('%')
            return render_template('modeltraining.html', msg=msg)
